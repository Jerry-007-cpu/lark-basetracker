...
用户明确说“保存、绑定、以后继续追踪”时，把当前表保存为有名称的本地追踪源。清单默认保存在 `~/.config/lark-basetracker/sources.json`，权限为 `0600`；不要把链接或清单内容显示在聊天正文中。

```bash
python3 scripts/organize_jobs.py source-add --name "<简称>" --kind lark \
  --location "<飞书链接>" --table-id "<table_id>"

python3 scripts/organize_jobs.py source-add --name "<简称>" --kind tencent \
  --location "<腾讯文档链接>" --sheet-id "<sheet_id>"
```
