...
同一次查询必须从第一条到最后一条保持同一种模板、字段顺序和链接格式。结果超过 20 条时使用 `--format compact` 或保留 `--format auto` 的自动紧凑格式；每条仍需包含全部约定展示字段，缺失值使用 `—`。禁止前半段输出详情、后半段只列名称，也禁止为了缩短回答而静默删除投递链接。需要分页时应明确标注页码和剩余数量，并保证每页模板一致。
